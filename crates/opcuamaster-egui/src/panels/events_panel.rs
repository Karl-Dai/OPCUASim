...
pub fn show(ui: &mut egui::Ui, model: &mut AppModel, backend: &BackendHandle) {
    ...
    ui.horizontal(|ui| {
        ...
        let can_subscribe = has_conn && !state.source_node_id.is_empty() && !state.subscribed;
        let sub_resp = ui.add_enabled(
            can_subscribe,
            egui::Button::new(if state.subscribed { "✓ 已订阅" } else { "📡 订阅" }),
        );
        if sub_resp.clicked() {
            if let Some(ref conn_id) = state.selected_conn {
                backend.send(UiCommand::SubscribeEvents {
                    conn_id: conn_id.clone(),
                    source_node_id: state.source_node_id.clone(),
                });
                state.subscribed = true;
            }
        }

        let can_unsub = state.subscribed;
        let unsub_resp = ui.add_enabled(
            can_unsub,
            egui::Button::new("⛔ 取消"),
        );
        if unsub_resp.clicked() {
            if let Some(ref conn_id) = state.selected_conn {
                backend.send(UiCommand::UnsubscribeEvents {
                    conn_id: conn_id.clone(),
                });
                state.subscribed = false;
            }
        ...
        if clear_resp.clicked() {
            if let Some(ref conn_id) = state.selected_conn {
                backend.send(UiCommand::ClearEvents {
                    conn_id: conn_id.clone(),
                });
                state.items.clear();
            }
...
pub fn apply_event_items(state: &mut EventsPanelState, items: Vec<EventItemDto>, full: bool) {
    if full {
        state.items = items;
    } else {
        state.items = items;
